...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Admin /</span> Edit Stok</h4>
        <!-- DataTable with Buttons -->
        <div class="card">
            <div class="card-datatable table-responsive pt-0">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Edit Data Stok</h5>
                    <!-- Move the button to the right using ml-auto -->
                </div>
                <div class="card-body">
                    <form action="{{ route('stok.update', ['stok' => $data->idStok]) }}" id="basic-form" method="post"
                        ...
                        <div class="mb-3">
                            ...
                            <select name="idBahan" id="select2basicproduct"
                                ...
                                @foreach ($bahan as $item)
                                    <option value="{{ $item->idBahan }}" @if ($item->idBahan == $data->idBahan) Selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
